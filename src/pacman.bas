...
50 gosub 3340:gosub 1600:gosub 1860:gosub 6000:nw%=1:cs=.:vol 5,10:ma%=4:gosub 4630:d$="{up}{rght}{down}{left}"
60 dim pt%(7):pt%(.)=7:pt%(1)=20:pt%(2)=7:pt%(3)=20:pt%(4)=5:pt%(5)=20:pt%(6)=5:pt%(7)=9999
70 dim tg%(4,1):tg%(1,.)=25:tg%(1,1)=-3:tg%(2,.)=2:tg%(2,1)=-3:tg%(3,.)=27:tg%(3,1)=25:tg%(4,.)=.:tg%(4,1)=25
...
110 sound 4,8000,5,2,.,1500,1,.:return
120 cursor 30,7:print "{lblu}"+right$("000000"+mid$(str$(cs),2),6):gosub 4850:sh%=.:hu=ti:return
...
150 if fr%=. and sg%<>ss% then sound 1,sf%,32767,2,sf%/2,700,2,2000:ss%=sg%
160 if ct-ft >= 0.25 then begin
170   fp%=fp%+1:if fp%=2 then fp%=.
180   c@&(1,2)=fp%:c@&(25,2)=fp%:c@&(1,16)=fp%:c@&(25,16)=fp%
190   ft=ct
200 bend
210 rem if ca<>0 then 330
220 if an%(.)=. then sn%=.:goto290
230 ct=ti
240 if ct-lt >= 0.05 then begin
250  if ac(.,1)=90  then sn%=sn%+1:if sn%>1 then sn%=.
260  if ac(.,1)=270 then sn%=sn%+2:if sn%>2 then sn%=.
270  if ac(.,1)=.   then sn%=sn%+3:if sn%>3 then sn%=.
280  if ac(.,1)=180 then sn%=sn%+4:if sn%>4 then sn%=.
290  sprsav sp$(sn%),.
300  lt=ct
310 bend
320 return
330 rem sprite to sprite collision handler
340 cm%=bump(1):co%=co% or cm%:if xx%=. then xx%=((co% and 1)=.)+1
350 return:rem 1540
360 rem init screen
370 border0: background0:color1:fort%=.to6:movspr t%,.#.:spritet%,.:next:dc%=.
380 gd%=.:gc%=.:ld=ti:fr%=.:gs%=100:fa%=.:fu%=.:fs%=.:fd%=.:mc%=.:ce%=1:gosub 5900
...
750 for t%=1 to 4:ge%(t%)=.:gr%(t%)=.:gb%(t%)=.:rb%(t%)=.:next t%
760 sn%=.:sprsav sp$(sn%),.
770 sprsav sp$(6),1:sprsav sp$(7),2:sprsav sp$(8),3:sprsav sp$(9),4
780 sprite .,1,ac(.,.):movspr .,ac(.,1)#ac(.,2):movspr .,ac(.,3),ac(.,4)
790 sprite 1,1,ac(1,.):movspr 1,ac(1,1)#ac(1,2):movspr 1,ac(1,3),ac(1,4)
800 sprite 2,1,ac(2,.):movspr 2,ac(2,1)#ac(2,2):movspr 2,ac(2,3),ac(2,4)
810 sprite 3,1,ac(3,.):movspr 3,ac(3,1)#ac(3,2):movspr 3,ac(3,3),ac(3,4)
820 sprite 4,1,ac(4,.):movspr 4,ac(4,1)#ac(4,2):movspr 4,ac(4,3),ac(4,4)
830 rem ready!
840 t%=bump(1):co%=.:xx%=.:cursor 11,12:print"{yel}ready!":gosub 4560
850 rem if new game, play music, else wait briefly
860 if nw%=1 then gosub 3310:nw%=.:sleep4.0:goto880
870 sleep 2
880 cursor 11,12:print"      "
890 rem movement loop
900 rem current speed and original speed
910 pc%=.:gt=ti:gm%=.:ut=ti:sh%=.:hu=ti:ss%=-1
920 fort%=.to4
930  ac(t%,2)=vg
932  if t%=. then ac(t%,2)=vp
934  ac(t%,9)=ac(t%,2)
940  ac(t%,7)=rsppos(t%,.):ac(t%,8)=rsppos(t%,1)
950 nextt%
960 b$="{left}"
965 if ti-ut<0.02 then 965
966 ut=ti
970 for ca%=. to ma%
980  if ca%=. then j%=joy(3) and 15:if (j% and 1)=1 then b$=mid$(d$,(j%+1)/2,1)
990 if ca%=. and fr%=. and pc%<7 and ti-gt>=pt%(pc%) then pc%=pc%+1:gt=ti:gm%=pc% and 1:gosub 3780
1000 if ca%=. and fr%=1 and ti-fz>=du% then fr%=.:gt=gt+(ti-fz):gosub 4460
1010 if ca%=. and fr%=1 and ti-fz>=wa then gosub 4520
1020  ac(ca%,7)=rsppos(ca%,.):ac(ca%,8)=rsppos(ca%,1)
1025  if ca%=. and (fu%=1 or fs%=1) then gosub 5700
1030  px%=ac(ca%,7)-22:py%=ac(ca%,8)-50
1040  rem check for tunnel
1050  if px%>219 and py%=79 then px%=8 :movspr ca%,px%,py%+50:goto1070
1060  if px%=.   and py%=79 then px%=216:movspr ca%,px%,py%+50
1070  tx%=int(px%/8):ty%=int((py%+8)/8)
1080  ai%=mod(px%,8)=. and mod(py%-7,8)=.
...
1210     if dc%=. then mc%=1
1220    if b$="{left}" and ml%<>6 then ac(.,1)=270
1230    if b$="{rght}" and mr%<>6 then ac(.,1)=90
1240    if b$="{up}" and mu%<>6 then ac(.,1)=.
1250    if b$="{down}" and md%<>6 then ac(.,1)=180
1260   bend
1270   no%=.
1280   if ac(ca%,1)=270 and ml%=6 then no%=1
1290   if ac(ca%,1)=90  and mr%=6 then no%=1
1300   if ac(ca%,1)=.   and mu%=6 then no%=1
1310   if ac(ca%,1)=180 and md%=6 then no%=1
1320   if ca%=. then begin
1330    ac(ca%,2)=ac(ca%,9):an%(ca%)=1:if no%=1 then ac(ca%,2)=.:an%(ca%)=.
1340   bend
1350 if ca%>. and ca%<5 and ai%=-1 then gosub 3480
1351 if ca%>. and ca%<5 and ge%(ca%)=. then begin
1352  ac(ca%,2)=vg
...
1357 bend
1360  dx%=.:dy%=.:qx%=mod(px%,8):qy%=mod(py%-7,8)
1370  if ac(ca%,1)=90  then dx%=8-qx%
1380  if ac(ca%,1)=270 then dx%=qx%-8:if qx%>. then dx%=-qx%
1390  if ac(ca%,1)=.   then dy%=qy%-8:if qy%>. then dy%=-qy%
1400  if ac(ca%,1)=180 then dy%=8-qy%
1410  if ac(ca%,2)>. then movspr ca%,px%+22,py%+50 to px%+22+dx%,py%+50+dy%,ac(ca%,2)
1420 bend
1430 next ca%
1431 if sh%=1 and ti-hu>=0.1 then gosub 120
1432 if mc%=1 then 1450
1435 if xx%=1 then xx%=.:gosub 4230:co%=.:if dth%=1 then dth%=.:goto 1540
1440 goto 970
1450 rem cleared screen
1452 if sh%=1 then gosub 120
1455 for t%=.to4:movspr t%,.#.:next t%:sound clr:sleep 1
1460 for t%=.to5:sprite t%,.:next t%:fu%=.:fs%=.
1470 forx%=1to4
1480  edma 3,80*25,.,$1f800
1490  sleep 0.25
...
1545 for t%=. to 4:movspr t%,.#.:next t%:sleep 0.5
1550 fu%=.:fs%=.:sprite 5,.:sound 1,1,1:sound 5,1,1:fort%=1to4:spritet%,.:next:sleep 0.2
1560 q%=.
1562 for t%=10 to 12
1564  sprsav sp$(t%),.
1566  for z%=1 to 4
1568   q%=q%+1:sound 4,24000-q%*1800,5,2,.,1200,1,.:sleep 0.09
1570  next z%
1572 next t%
1574 sprite .,.:sleep 0.3
1580 gd%=1:gc%=.:ld=ti:fr%=.:ce%=.:sound 5,1,1:lv%=lv%-1:gosub 4560:if lv%<. then 4600
1590 gosub 3340:goto 740
1600 rem character defs
1610 chardef 46,.,.,.,24,24,.,.,.                : rem dots
1620 chardef 66,144,144,144,144,144,144,144,144 : rem l vert bar
1630 chardef 67,.,.,255,.,.,255,.,. : rem horiz double bar
1640 chardef 85,.,.,31,32,32,35,36,36: rem joy - shft-o
1650 chardef 75,36,36,196,4,4,248,.,.: rem curve - up to left
1660 chardef 74,36,36,35,32,32,31,.,.: rem curve - up to right
1670 chardef 73,36,36,39,32,32,31,.,.: rem rpen - shft-p
1680 chardef 115,9,9,241,1,1,241,9,9:rem v bar, left connection
1690 chardef 107,144,144,143,128,128,143,144,144:rem - - alt-q
1700 chardef 114,.,.,255,.,.,231,36,36
1710 chardef 113,36,36,231,.,.,255,.,.
1720 chardef 90,.,.,3,4,4,3,.,.: rem sound  shft-z
1730 chardef 88,.,.,192,32,32,192,.,.: rem tron - shft-x
1740 chardef 65,24,. ,. ,. ,. ,. ,.,.: rem atn : shft-a
1750 chardef 91,36,36,196,4,4,196,36,36:rem resume - alt-dbl quote
1760 chardef 81,.,24,60,126,126,60,24,.:rem dec - shft-o
1770 chardef 35,9,9,9,9,9,9,9,9: rem righ border
1780 chardef 87,36,36,36,36,36,36,36,36: rem trap - shft-w
1790 chardef 76,144,144,143,128,128,127,.,.: rem rgraphic - shft l
1800 chardef 79,.,.,127,128,128,143,144,144: rem joy - shft-o
1810 chardef 122,9,9,241,1,1,254,.,. : rem sqr - shft-@
1820 chardef 80,.,.,254,1,1,241,9,9: rem rpen - shft-p
1830 chardef 73,.,.,248,4,4,196,36,36: rem rpen - shft-p
...
3370 ac(.,.)=7  :ac(1,.)=2  :ac(2,.)=4  :ac(3,.)=3  :ac(4,.)=18
3380 ac(.,1)=270:ac(1,1)=90 :ac(2,1)=90 :ac(3,1)=90 :ac(4,1)=270
3390 ac(.,2)=.  :ac(1,2)=.  :ac(2,2)=.  :ac(3,2)=.  :ac(4,2)=.
3400 ac(.,3)=126:ac(1,3)=126:ac(2,3)=110:ac(3,3)=126:ac(4,3)=142
3410 ac(.,4)=177:ac(1,4)=113:ac(2,4)=129:ac(3,4)=129:ac(4,4)=129
3420 return:rem 129
3430 mt=ti
3440 et=ti-mt
...
3510 if ty%=9 and tx%=13 then ac(ca%,1)=.:return
...
3550 if gm%=. then gx%=tg%(ca%,.):gy%=tg%(ca%,1):goto 3680
3560 rem chase targets
3570 if ca%=1 then gx%=ac(.,5):gy%=ac(.,6):goto 3680
3580 ux%=.:uy%=.:pd=ac(.,1)
3590 if pd=. then ux%=-1:uy%=-1
3600 if pd=90 then ux%=1
3610 if pd=180 then uy%=1
3620 if pd=270 then ux%=-1
3630 if ca%=2 then gx%=ac(.,5)+4*ux%:gy%=ac(.,6)+4*uy%:goto 3680
3640 if ca%=3 then ex%=ac(.,5)+2*ux%:ey%=ac(.,6)+2*uy%:gx%=2*ex%-ac(1,5):gy%=2*ey%-ac(1,6):goto 3680
3650 dd%=(tx%-ac(.,5))*(tx%-ac(.,5))+(ty%-ac(.,6))*(ty%-ac(.,6))
3660 if dd%>64 then gx%=ac(.,5):gy%=ac(.,6):goto 3680
3670 gx%=tg%(4,.):gy%=tg%(4,1)
3680 rem pick the open non-reverse direction nearest the target
3690 bd%=-1:bv%=32767
3700 if mu%<>6 and od%<>.   then nx%=tx%:ny%=ty%-1:gosub 3770:if nv%<bv% then bv%=nv%:bd%=.
...
4000 if rv%=1 and rb%(ca%)>. then rb%(ca%)=rb%(ca%)-1:rr%=1
4002 if rr%=1 and rb%(ca%)=. then 4020
...
4120 ge%(ca%)=.:gr%(ca%)=.:ac(ca%,1)=90:ac(ca%,2)=vg:rb%(ca%)=2+2*int(rnd(1)*2)
4130 sprsav sp$(5+ca%),ca%:sprite ca%,1,ac(ca%,.)
4140 goto 3840
4150 rem frightened: random turn at each tile
4160 k%=.
4170 if mu%<>6 and od%<>. then k%=k%+1:cd%(k%)=.
...
4210 if k%=. then ac(ca%,1)=od%:return
4220 ac(ca%,1)=cd%(int(rnd(1)*k%)+1):return
4230 rem collision resolve: lethal contacts take priority
4240 dth%=.
4250 for t%=1 to 4
4260 if (co% and (2^t%))=. then 4300
4270 if abs(rsppos(t%,.)-rsppos(.,.))>12 or abs(rsppos(t%,1)-rsppos(.,1))>12 then 4300
4280 if ge%(t%)=1 then 4300
4290 if gb%(t%)=. then dth%=1
4300 next t%
4310 if dth%=1 then return
4320 for t%=1 to 4
4321 if (co% and (2^t%))=. then 4332
4322 if abs(rsppos(t%,.)-rsppos(.,.))>12 or abs(rsppos(t%,1)-rsppos(.,1))>12 then 4332
4323 if ge%(t%)=1 or gb%(t%)=. then 4332
4324 gs%=gs%*2:cs=cs+gs%:gosub 120:ge%(t%)=1:gb%(t%)=.:sprsav sp$(13),t%:sprite t%,1,1:ac(t%,2)=1.5
4325 nc%=nc%+1:sound 6,20000,14,.,.,2200,2,2048:if nc%>4 then nc%=4
4326 sprsav sp$(13+nc%),6:movspr 6,ac(.,7),ac(.,8)-10:sprite 6,1,3
4327 pd=0.75:gosub 5750:sprite 6,.
4332 next t%
4334 return
4380 rem power pellet: frighten the ghosts
4385 if fr%=1 then gt=gt+(ti-fz)
4390 gc%=gc%+1:ld=ti:fr%=1:fz=ti:gs%=100:nc%=.:sound 1,1,1:ss%=-1
4392 if du%>. then sound 5,8000,360,2,2000,1200,2,2048
4395 if du%=. then fr%=.
4400 for t%=1 to 4
4410 if ge%(t%)=1 then 4440
4415 if gr%(t%)=. then 4430
4420 a%=ac(t%,1)+180:if a%>=360 then a%=a%-360
4425 ac(t%,1)=a%
4430 if du%=. then 4440
4435 ac(t%,2)=vf:sprite t%,1,6:gb%(t%)=1
4440 next t%
4450 return
4460 rem frightened over: colours and speeds back
4470 for t%=1 to 4
4480 if gb%(t%)=1 then ac(t%,2)=vg:sprite t%,1,ac(t%,.):gb%(t%)=.
4490 next t%
...
4570 if lv%>. then for t%=1 to lv%:t@&(t%,24)=60:c@&(t%,24)=7:next t%
...
4605 sleep 3
4610 if cs>. and (hc%<10 or cs>hv(hc%)) then gosub 5500
4615 print chr$(147):cs=.:lv%=2:le%=1:nw%=1:xl%=.:goto 4650
...
4660 border .:background .:print chr$(147):q$=chr$(34)
4661 for t%=. to 7:movspr t%,.#.:sprite t%,.:next t%
...
4664 aw%=.:gosub 4700:bc%=.:gosub 4990
4670 if (joy(3) and 128)>. then 4670
...
4740 for t%=1 to 4:movspr t%,.#.:movspr t%,86,81+t%*24:sprite t%,1,ac(t%,.):next t%
...
4950 if hc%=. then cursor 13,10:print"{wht}no scores yet"
4960 return
...
4984 for i%=4 to 22:cursor .,i%:print"                                        ";:next i%
4986 return
4990 cursor 11,23
4992 if bc%=. then print"{red}press fire to play";:return
4994 print"{yel}press fire to play";:return
5000 rem load and sort the scores seq file
5010 hc%=.:hs=.:dopen#2,"scores"
...
5070 if st=. then 5040
5080 dclose#2:if hc%>. then hs=hv(1)
5090 return
5200 rem parse one name,date,score record
5210 p%=instr(r$,","):if p%=. then return
5220 n$=left$(r$,p%-1):x$=mid$(r$,p%+1):p%=instr(x$,","):if p%=. then return
5230 sd$=left$(x$,p%-1):if left$(sd$,1)=" " then sd$=mid$(sd$,2)
5240 sc=val(mid$(x$,p%+1)):if n$="" or sc<. then return
5245 gosub 5450
...
5670 sprsav sp$(17+fi%),5:movspr 5,.#.:movspr 5,126,177:sprite 5,1,zc%(fi%)
5680 fu%=1:fs%=.:fe=ti+9+rnd(1):fv%=pv%(fi%):return
5700 rem expire or collect the visible item
5710 if ti>=fe then fu%=.:fs%=.:sprite 5,.:return
5715 if fs%=1 then return
5720 if abs(ac(.,7)-126)>12 or abs(ac(.,8)-177)>12 then return
5730 fu%=.:fs%=1:fe=ti+1:sprsav sp$(25+fi%),5:sprite 5,1,zc%(fi%):cs=cs+fv%:gosub 120
...
5912 if le%>=2 then vp=0.95:vg=0.89:vf=0.445:du%=5:bw=2.25:p3%=.:p4%=50
5914 if le%>=3 then vp=1:vg=0.94:vf=0.47:du%=4:bw=2:p3%=.:p4%=.
...
5921 if le%=17 or le%>=19 then du%=.:bw=.
...
5930 wa=du%-bw:if wa<. then wa=.
5940 pt%(.)=7:pt%(1)=20:pt%(2)=7:pt%(3)=20:pt%(4)=5:pt%(5)=20:pt%(6)=5:pt%(7)=9999
5950 if le%>=2 then pt%(5)=1033:pt%(6)=1
5960 if le%>=5 then pt%(.)=5:pt%(2)=5:pt%(5)=1037
5970 return
6000 rem item character definitions and sprite shapes
6010 chardef 92,12,18,4,26,60,90,36,.:rem cherry
6020 chardef 93,36,126,60,126,60,24,24,.:rem strawberry
6030 chardef 94,8,4,60,126,126,126,60,.:rem orange
6040 chardef 95,8,4,52,126,126,126,60,.:rem apple
6050 chardef 96,24,60,90,126,90,126,60,24:rem melon
6060 chardef 97,24,90,126,60,126,90,24,.:rem galaxian
6070 chardef 98,24,60,60,126,126,126,24,.:rem bell
6080 chardef 99,60,66,60,24,24,30,24,.:rem key
